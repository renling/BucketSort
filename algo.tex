...
\begin{proof}
We first describe the simulator $\Sim$. 
...
Then, 
the simulator chooses random loads $\vec{K} = (K_0,K_1,\ldots,K_{B-1})$, where $K_i$ is the load of the real elements in the $i$th bin. This is done by simply throwing $n$ elements into $B$ bins (``in the head''). If there is some $i$ for which $K_i > Z$ then the simulator aborts. The removal of the dummy elements is equivalent to the revealing of these loads. Specifically, given 
...
We next show that the output of the algorithm is a random permutation, conditioned of the access pattern. As we previously described, it is actually enough to condition on the vector of random loads $\vec{K} = (K_0,\ldots,K_{B-1})$. We show that given any such vector, all permutations are possible.  

For a particular load $\vec{k}=(k_0, k_1, \ldots, k_{B-1})$, there are ${n \choose k_0,\ldots,k_{B-1}}$ ways to distribute the elements into the bins while achieving the vector of loads $\vec{k}$. Then, each bin is individually permuted. 
